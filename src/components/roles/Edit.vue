...
export default {
  name: "Add",
  computed: {
    ...mapGetters(["allModels", "allMaps", "getRole"]),
  },
  data() {
    return {
      name: "",
  ...
  mounted() {
    this.$store
      .dispatch(`getRole`, this.$route.params.id)
      .then((res) => {
        this.name = res.data.role.name;
        this.discription = res.data.role.description;
        for (let i = 0; i < res.data.role.permissions.length; i++) {
          this.permissions.push(res.data.role.permissions[i].name);
        }
      })
      .catch((err) => {
        console.log(err);
